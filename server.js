...
function normalizeSportMonksGame(fx){
  ...
  const scoreOf = p => scores.find(x=>x.participant_id===p.id && ["CURRENT","2ND_HALF","1ST_HALF"].includes(String(x.description||"").toUpperCase()))?.score?.goals ?? "";
  const state = String(fx.state?.state || fx.state?.name || "").toUpperCase();
  const live = ["LIVE","HT","1ST_HALF","2ND_HALF"].includes(state);
  const finished = ["FT","AET","FT_PEN","FULLTIME"].includes(state);
  return {
    ...
    status:fx.state?.name || state || "", state:live?"in":finished?"post":"pre", live,
...
app.get("/api/games", async (req,res)=>{
  ...
  for(const g of out){
    ...
    const old=map.get(k);
    if(!old) map.set(k,g);
    else if(g.source==="SportMonks") map.set(k,{...old,...g});
    else if(g.source==="API-Football" && old.source==="ESPN") map.set(k,{...old,...g});
  }
