...
function makeInsights(stats) {
  const byStat = {};
  stats.forEach(s => {
    const k = String(s.label || "").toLowerCase();
    if (!byStat[k]) byStat[k] = [];
    byStat[k].push(s);
  });

  const insights = [];
  function compare(names, label) {
    const key = Object.keys(byStat).find(k => names.some(n => k.includes(n)));
    if (!key || byStat[key].length < 2) return null;
    const [a,b] = byStat[key];
    const av = valueToNumber(a.value), bv = valueToNumber(b.value);
    const diff = Math.abs(av-bv);
    if (diff <= 0) return null;
    const leader = av > bv ? a.team : b.team;
    return { label, leader, aTeam:a.team, bTeam:b.team, aValue:a.value, bValue:b.value, diff };
  }

  const shots = compare(["shots total", "total shots", "chutes"], "Finalizações");
  const corners = compare(["corner", "escanteio"], "Escanteios");
  const possession = compare(["possession", "posse"], "Posse de bola");
  const target = compare(["shots on goal", "shots on target", "chutes no gol"], "Chutes no gol");

  [shots, corners, possession, target].filter(Boolean).forEach(x => insights.push(x));

  let trend = "Sem leitura forte ainda";
  if (shots && shots.diff >= 4) trend = `${shots.leader} está finalizando mais`;
  if (corners && corners.diff >= 3) trend = `${corners.leader} pressiona pelos escanteios`;
  if (target && target.diff >= 2) trend = `${target.leader} chega melhor no gol`;
...
app.get("/api/game-details", async (req, res) => {
  ...
  try {
    ...
    res.json({
      ...
      analysis: makeInsights(teamStats)
    });
  } catch (e) {
...
app.listen(PORT, () => console.log(`Meu Futebol Live V5 rodando na porta ${PORT}`));
