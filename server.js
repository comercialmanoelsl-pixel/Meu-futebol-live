...
function makeAnalysis(stats, events, game){
  ...
  let hp=35, ap=35;
  hp += Math.min(homeShots*2.1,28)+Math.min(homeTarget*3.2,22)+Math.min(homeCorners*2.6,22)+Math.min(homePoss*.22,16)+(homeScore<awayScore?8:0);
  ap += Math.min(awayShots*2.1,28)+Math.min(awayTarget*3.2,22)+Math.min(awayCorners*2.6,22)+Math.min(awayPoss*.22,16)+(awayScore<homeScore?8:0);
  ...
  const level=pressure>=80?"MUITO ALTA":pressure>=65?"ALTA":pressure>=45?"MODERADA":"BAIXA";
  const reasons=[];
  ...
  const trend=pressure>=75?`${leader} pressionando forte e criando mais perigo.`:pressure>=55?`${leader} tem leve superioridade ofensiva.`:"Sem tendência forte detectada.";
  return {homeTeam:t.home,awayTeam:t.away,homePressure:hp,awayPressure:ap,pressure,pressureTeam:leader,pressureLevel:level,trend,reasons,cornerTrend:homeCorners+awayCorners>=8?"Alta":homeCorners+awayCorners>=4?"Moderada":"Baixa",shotTrend:homeShots+awayShots>=18?"Alta":homeShots+awayShots>=10?"Moderada":"Baixa",raw:{homeShots,awayShots,homeTarget,awayTarget,homeCorners,awayCorners,homePoss,awayPoss}};
}
...
app.get("/api/game-details", async (req,res)=>{
  try{
    ...
    res.json({ok:true,source:"API-Football",fixtureId,teamStats,events,lineups,analysis});
  }catch(e){res.status(500).json({ok:false,error:String(e.message||e)});}
});

app.listen(PORT, () => {
  console.log("DhuniorStats rodando na porta " + PORT);
});
