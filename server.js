...
function normalizeStats(resp){
  ...
  (resp||[]).forEach(tb=>{
    ...
    (tb.statistics||[]).forEach(s=>rows.push({team,label:String(s.type||""),value:s.value ?? ""}));
  });
  return rows;
}
...
function makeTrendOnly(game, pregame){
  return {
    dataType:"trend",
    hasEnoughData:pregame.hasEnoughData,
    confidence:pregame.confidence,
    reading:pregame.hasEnoughData ? `Tendência contextual: ${pregame.pressureTeam} aparece melhor pelo histórico disponível.` : "Sem dados reais suficientes para tendência.",
    pressureTeam:pregame.pressureTeam || "",
    base:pregame.base || []
  };
}
...
app.get("/api/game-details", async (req,res)=>{
  try{
    ...
    const trend = makeTrendOnly(game, pregame);

    const analysis = {
      live: live || {dataType:"none",hasEnoughData:false,reading:"Sem estatísticas ao vivo suficientes."},
      trend,
      pregame,
      displayStats: live || null,
...
app.listen(PORT, () => console.log("DhuniorStats V15 Stats Reais rodando na porta " + PORT));
