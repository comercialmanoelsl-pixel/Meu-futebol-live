...
function makeLiveFromRows(stats, game){
  ...
  const finalHome=num(find(hs,['total shots','shots total','shots'])); const finalAway=num(find(as,['total shots','shots total','shots']));
  const targetHome=num(find(hs,['shots on goal','shots on target','shots on'])); const targetAway=num(find(as,['shots on goal','shots on target','shots on']));
  const cornerHome=num(find(hs,['corner'])); const cornerAway=num(find(as,['corner']));
  const possHome=num(find(hs,['ball possession','possession'])); const possAway=num(find(as,['ball possession','possession']));
  const has=!!(finalHome||finalAway||targetHome||targetAway||cornerHome||cornerAway||possHome||possAway); if(!has) return null;
  const hp=finalHome*2+targetHome*4+cornerHome*2+possHome*.25; const ap=finalAway*2+targetAway*4+cornerAway*2+possAway*.25; const leader=hp>=ap?game.home.name:game.away.name;
