...
const INTERNAL_CONTEXT = {
  ...
  strongTeams: ["Bodo/Glimt","Palmeiras","Flamengo","Botafogo","Manchester City","Real Madrid","Barcelona","Bayern Munich","PSG","Liverpool","Arsenal"]
};
...
app.get("/api/health", (req,res)=>res.json({
  ok:true,
  providers:{apiFootball:!!API_FOOTBALL_KEY, espn:true, news:!!NEWS_API_KEY, sportMonks:!!SPORTMONKS_KEY}
}));
...
app.get("/api/games", async (req,res)=>{
  ...
  let games=[...map.values()].sort((a,b)=>new Date(a.date)-new Date(b.date));
  const quality = String(req.query.quality || "all");
  if(quality==="stats"){
    games = games.filter(g => g.fixtureId);
  }
...
function makeLive(stats){
  ...
  return {
    ...
    pressureTeam:leader,
    pressureIndex: Math.min(100, Math.round(Math.max(hp, ap))),
    ...
    base:[`Finalizações ao vivo: ${finalHome} x ${finalAway}.`,`Chutes no gol ao vivo: ${targetHome} x ${targetAway}.`,`Escanteios ao vivo: ${cornerHome} x ${cornerAway}.`,`Posse ao vivo: ${possHome || "-"}% x ${possAway || "-"}%.`]
  };
}

function estimateFromContext(game, pregame, ctx){
  const home = game.home?.name || "Mandante";
  const away = game.away?.name || "Visitante";
  let homePower = 50 + (pregame.homeContextScore || 0);
  let awayPower = 50 + (pregame.awayContextScore || 0);

  for(const t of INTERNAL_CONTEXT.strongTeams){
    if(norm(t)===norm(home)) homePower += 12;
    if(norm(t)===norm(away)) awayPower += 12;
  }

  const internal = getInternalNotes(game);
  if(internal.notes.some(n=>n.includes("altitude")) && game.state === "pre") homePower += 10;

  const total = Math.max(1, homePower + awayPower);
  const homeShare = homePower / total;
  const awayShare = awayPower / total;

  const baseFinal = game.live ? 18 : 22;
  const finalHome = Math.max(4, Math.round(baseFinal * homeShare));
  const finalAway = Math.max(4, Math.round(baseFinal * awayShare));
  const targetHome = Math.max(1, Math.round(finalHome * 0.38));
  const targetAway = Math.max(1, Math.round(finalAway * 0.38));
  const cornerHome = Math.max(1, Math.round(finalHome * 0.42));
  const cornerAway = Math.max(1, Math.round(finalAway * 0.42));
  const possHome = Math.round(45 + (homeShare-0.5)*45);
  const possAway = 100 - possHome;
  const leader = homePower >= awayPower ? home : away;

  return {
    dataType:"estimated",
    hasEnoughData: pregame.hasEnoughData,
    confidence: pregame.hasEnoughData ? pregame.confidence : "Baixa",
    reading: pregame.hasEnoughData ? `${leader} tem melhor tendência estimada pelo contexto/histórico disponível.` : "Estimativa limitada: faltam dados reais suficientes.",
    pressureTeam: pregame.hasEnoughData ? leader : "",
    pressureIndex: pregame.hasEnoughData ? Math.min(100, Math.round(Math.max(homeShare, awayShare)*100)) : 0,
    finalizations:{home:finalHome,away:finalAway},
    shotsOnGoal:{home:targetHome,away:targetAway},
    corners:{home:cornerHome,away:cornerAway},
    possession:{home:possHome,away:possAway},
    base:[
      "Estatísticas abaixo são estimativas contextuais, não dados ao vivo.",
      ...pregame.base
    ]
  };
}
...
async function fetchNewsContext(game){
  if(!NEWS_API_KEY) return {enabled:false, items:[], base:[]};
  try{
  ...
  }catch(e){ return {enabled:true, items:[], base:[]}; }
}
...
app.get("/api/game-details", async (req,res)=>{
  try{
    ...
    let pregame, playersHome=[], playersAway=[], newsContext={items:[],base:[]};
    ...
    if(game?.home?.id && game?.away?.id){
      ...
      pregame = {
        hasEnoughData:false,confidence:"Baixa",evidenceCount:0,homeContextScore:0,awayContextScore:0,pressureTeam:"",
    ...
    const estimated = estimateFromContext(game, pregame, {});
    const displayStats = live || estimated;

    const analysis = {
      live: live || {hasEnoughData:false, reading:"Sem estatísticas ao vivo suficientes para leitura forte."},
      estimated,
      pregame,
      displayStats,
      contextSource:"API-Football + ESPN + base interna + notícias opcionais",
      main: live || estimated
    };
...
app.listen(PORT, () => console.log("DhuniorStats V14 Estatísticas rodando na porta " + PORT));
