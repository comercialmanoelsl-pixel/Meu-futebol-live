...
function renderCoverage(){
  if(!coverageData){
    $("mainPanel").innerHTML=`<div class="title">Cobertura</div><div class="dataNotice">Carregando cobertura...</div>`;
    return;
  }
  const c=coverageData.coverage||{};
  const badge=(v)=>`<span class="formationPill">${v?"✅":"❌"}</span>`;
  const items=[
    ["Fixture",c.fixture],["Participants",c.participants],["Scores",c.scores],
    ["Events",c.events],["Statistics",c.statistics],["Lineups",c.lineups],
    ["Formations",c.formations],["Coaches",c.coaches],["Sidelined",c.sidelined],
    ["Recent matches",c.recentMatches],["Players",c.players],["Odds",c.odds]
  ];
  $("mainPanel").innerHTML=`<div class="title">Cobertura real da partida</div>
    <div class="aiBox"><div class="aiText">Coverage Score: ${c.score||0}% · Estado: ${esc(coverageData.state||"-")}</div></div><br>
    <div class="compactGrid">${items.map(([k,v])=>`<div class="compactCard"><b>${k}</b><br>${badge(v)}</div>`).join("")}</div>
    <br><div class="title">IDs resolvidos</div>
    <div class="base">
      <div>SportMonks: ${esc(coverageData.ids?.sportmonksId||"-")}</div>
      <div>API-Football: ${esc(coverageData.ids?.apiFootballId||"-")}</div>
      <div>Casa ID: ${esc(coverageData.ids?.homeId||"-")}</div>
      <div>Fora ID: ${esc(coverageData.ids?.awayId||"-")}</div>
    </div>
    <br><div class="title">Notas</div>
    <div class="dataNotice">${(coverageData.notes||[]).map(esc).join("<br>")||"Sem alertas."}</div>`;
}
