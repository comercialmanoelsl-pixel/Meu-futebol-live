...
async function loadDetails(g){
  ...
  try{
    ...
    const resolvedSmId = await resolveSportMonksIdForSelectedGame(g);
    if(resolvedSmId){
      try{
        const smd=await fetch(`/api/sm/components/fixture/${encodeURIComponent(resolvedSmId)}?_=${Date.now()}`).then(r=>r.json());
